...
void* list_iterator (void *_list_node){

	/*write your code here*/
	if(!_list_node) return NULL;
	dll_node_t *list_node = (dll_node_t *)_list_node;
	if(list_node->right)
		return (dll_node_t *)list_node->right;
	return NULL;    
}
...
int student_comparator (void *_studentobj1, void *_studentobj2){
	...
	if(!studentobj1 && !studentobj2) return 0; /* Same students*/
	if(!studentobj1 || !studentobj2) return -1; /* Anyone is NULL means students are different*/
	if(strncmp(studentobj1->name, studentobj2->name, 30) == 0 &&
...
main(int argc, char **argv){
    ...
    strncpy(student8->name, "Neeraja", strlen("Neeraja"));
